### This script generates the boxplots for the test of the relevance and full model on LOV benchmark.
...
for (metric in c(1:length(metrics))) {
    ...
    data <- read.table(paste0("../test-lov/output/results/test_",metrics[metric],"_",kcv,"_",norm,"_",basename(lovTestFile),".csv"), header=TRUE, sep = ",", stringsAsFactors = FALSE)
...
grid.arrange(plotMap, plotNdcg, plotErr, nrow = 1, left=textGrob("LOV Test", gp = gpar(fontface = "bold", cex = 1.1), rot=90))
dev.off()
