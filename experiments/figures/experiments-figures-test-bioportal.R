### This script generates the boxplots for the test of the relevance and full model on BioPortal benchmark.
...
kcv <- "10"
norm <- "sum"
bioportalTestFile <- "./test-bioportal/BioPortalTestSet"
trainFile <- "TrainingSet-shuffled"
filename <- "./output/boxplot_test_bioportal.tex"
dir.create(dirname(filename),showWarnings = FALSE);

# Generate boxplots
for (metric in c(1:length(metrics))) {
    
    # Read data for reference of first experiment
    validationReference <- read.table(paste0("../train/output/results/validation_",metrics[metric],"_",kcv,"_",norm,"_",trainFile,".csv"), header=TRUE, sep = ",", stringsAsFactors = FALSE)
    validationReference <- validationReference %>% filter(Model=="full-model.txt")
    
    # Read data for metric
    data <-  read.table(paste0("../test-bioportal/output/results/test_",metrics[metric],"_",kcv,"_",norm,"_",basename(bioportalTestFile),".csv"), header=TRUE, sep = ",", stringsAsFactors = FALSE)
    ...
    plot <- plot + geom_hline(yintercept=mean(colMeans(validationReference %>% select(yCol))), linetype="dashed", color = "blue", size = 1)
...
grid.arrange(plotMap, plotNdcg, plotErr, nrow = 1, left=textGrob("BioPortal Test", gp = gpar(fontface = "bold", cex = 1.1), rot=90))
dev.off()
